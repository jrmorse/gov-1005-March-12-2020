...
```{r}
train %>%
# we need to do a bootstrap early so that it will take the sample before all the group by.
# we want everyone in the sample, so we chose 115 people as the sample size
...
# This means that we can say with 95% certainty that the average income disparities between Boston commuters falls within this range.
```
...
train %>% 
  rep_sample_n(size = 115, replace = TRUE, reps = 1000) %>%
  group_by(replicate, treatment) %>% 
  summarise(attitude = mean(att_chg)) %>% 
  pivot_wider(names_from = treatment, values_from = attitude) %>% 
  mutate(Difference = Treated - Control) %>%
  select(replicate, Difference, everything()) %>% 
  pull(Difference) %>%
  quantile(c(0.025, 0.975))
